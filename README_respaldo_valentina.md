...
│   MongoDB Atlas                 │
│   BigData_ECommerce             │
│   6 colecciones (1 por categ.)  │
└───────────────┬─────────────────┘
                │ spark.read.format("mongodb")
                ▼
┌─────────────────────────────────┐
│   Apache Spark (main.py)        │
│   unionByName → filter → transform │
│   Reporte consolidado de mercado │
└─────────────────────────────────┘
```
...
```
├── main.py                          ← Orquestador Spark (integra los 6 scrapers)
...
└── books_toscrape/                  ← Avances semanales (S2–S8)
```
